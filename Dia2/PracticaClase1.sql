...
'8a05bcaf-1b04-4ec3-a9e6-c873fbd7130d', '15.23', '[10,20]',
array['rojo', 'verde', 'azul', 'amarillo'], 'Masculino'
);

select * from ejemplo;
SET lc_monetary = 'en_US.UTF-8'; 
SET lc_monetary = 'es_CO.UTF-8';
SELECT moneda FROM ejemplo;

--select gen_random_uuid();

create table usuarios(
	id serial primary key,  --uuid default gen_random_uuid()
	nombre varchar(100),
...
alter table usuarios add constraint fk_usuario foreign key(usuario) references credenciales(usuario);

insert into credenciales (usuario, password, user_rol)
select usuario, 'password' as passsword, 'usuario' as rol from usuarios;
...
-- ejemplo  alter table integrante add constraint check_edad check(edad > 9);
...
select substr(md5(random()::text), 1, 16) as passwordd;

alter table credenciales alter column password set default substr(md5(random()::text), 1, 16);

update credenciales set password = substr(md5(random()::text), 1, 16);

--##########################

create function generate_password (long integer)
returns text as $$
begin
	select substr(md5(random()::text), 1, long) as passwordd;
end;
$$ language plpgsql;
...
insert into usuarios (nombre, apellido, gmail, ip, ciudad, blog, usuario, siguiendo, seguidores)values
